...
【重要なルール】
- **存在しない文献・論文を存在するかのように引用することは絶対に禁止です。** 引用が必要な場合は「出典確認中」と明記してください。
- **提案する研究デザインが、提示されたRQの因果レベルと一致しているか必ず自己確認してください。不一致がある場合は修正してください。**
...
【重要なルール】
- **存在しない文献・論文を存在するかのように引用することは絶対に禁止です。** 架空の著者名やタイトルを捨造しないでください。「どのような文献を探すべきか」のアドバイスに留めてください。
...
- **提案する解析方法がアウトカムのデータ型（連続/二値/カテゴリカル等）と一致しているか必ず明示してください。**
- **観察研究で因果推論を示唆する解析を提案する場合は、その限界を明示してください。**
...
【重要なルール】
- **存在しない文献・論文を存在するかのように引用することは絶対に禁止です。**
...
【内部整合性確認】
研究目的・デザイン・解析方法の整合性を確認し、不整合があれば修正してください。
特に以下の点を事前にチェックすること：
- RQが示す因果レベルとデザインが一致しているか
- アウトカムのデータ型と選択された統計手法が適合しているか
- サンプルサイズが解析計画に対して十分か
...
- **存在しない文献・論文を存在するかのように引用することは絶対に禁止です。架空の著者名・タイトル・ジャーナル名・発行年を捐造しないでください。**
- 先行研究に言及する場合は、「〜に関する先行研究によれば」「既存のエビデンスでは」等の一般記述に留めること
- markdown形式で出力すること（#, ##, ###, -, ** を使用）
- **ガイドラインのチェックリストにユーザーが入力したメモ・方針がある場合、それらの内容を該当するセクションで必ず具体的に反映すること**（例：ユーザーが「サンプルサイズの根拠」に「G*Powerで算出、α=0.05, power=0.8」と書いていたら、その情報を「サンプルサイズの根拠」セクションに組み込む）
- チェックリストのメモは研究者本人の考えや方針であるため、尊重して計画書に統合すること`,searchQuerySuggestion:`あなたは医学・看護学領域の専門図書館員および文献検索のエキスパートです。
...
📌 **次のステップ**: では一旦アプリに戻ってください。
アプリの「回答を貼り付け」エリアに、上記の提案をコピー＆ペーストして「取り込む」ボタンを押してください。
」`,logicalAudit:`あなたは研究計画の論理整合性を監査する専門エンジンです。
教育的配慮は不要です。構造の矛盾のみを指摘してください。
...
- 二値アウトカムに不適切な統計手法を選択
- 多変量解析に対してサンプルサイズ不足
- RQに対して必要データが収集計画に含まれていない

【重要なルール】
- **存在しない文献・論文を存在するかのように引用することは絶対に禁止です。**
- 指摘は具体的に行い、曖昧な表現は避けてください。
- 修正提案には具体的な手法名や数値を含めてください。
...
Ⅴ. 期待される成果
看護師の業務負担を最適化しつつ、患者の安全な在宅移行を実現するためのエビデンスを提示する。`,logicalAudit:`🔎 研究整合性チェック結果

## 🚨 問題点
- リサーチクエスチョンが「標準化プロトコルは再入院を低減させるか？」と因果関係を示唆しているが、研究デザインが観察研究（STROBE準拠）であり、因果推論の主張レベルとデザインが不一致である。
- サンプルサイズの根拠（各群100例）が提示されているが、主要アウトカム（再入院の有無）に対するロジスティック回帰分析の独立変数の数を考慮すると、Events Per Variable（EPV）が不十分になる可能性がある。

## 🔧 修正提案
- RQの表現を「〜と関連があるか？」に修正するか、デザインを準実験研究（前後比較）に変更することを推奨。
- サンプルサイズの再算出：EPV≧10を確保するため、予測因子数に応じた最小イベント数を明記すること。

総合判定: 🟡 軽微な修正が望ましい — 修正推奨だが計画書作成は可能`},Ae="https://gemini.google.com/app";async function Ie(e){try{return await navigator.clipboard.writeText(e),!0}catch{const s=document.createElement("textarea");s.value=e,s.style.position="fixed",s.style.opacity="0",document.body.appendChild(s),s.select();try{return document.execCommand("copy"),document.body.removeChild(s),!0}catch{return document.body.removeChild(s),!1}}}function Oe(){window.open(Ae,"_blank","noopener,noreferrer")}function T(e){const{prompt:t,containerId:s,label:r="AI回答",expectJson:i=!1,placeholder:n="Geminiからの回答をここに貼り付けてください..."}=e;return`
...
...(続き)`:""}</div>
    ...
    `}function A(e,t,s,r=!1){const i=e.querySelector(".gemini-copy-btn"),n=e.querySelector(".gemini-open-btn"),o=e.querySelector(".gemini-response-input"),l=e.querySelector(".gemini-submit-btn"),c=e.querySelector(".gemini-copy-feedback");i&&i.addEventListener("click",async()=>{const d=await Ie(t);c&&(c.textContent=d?"✅ コピーしました！":"❌ コピーに失敗しました",c.style.opacity="1",setTimeout(()=>{c.style.opacity="0"},2500))}),n&&n.addEventListener("click",()=>{Oe()}),o&&l&&(o.addEventListener("input",()=>{l.disabled=!o.value.trim()}),l.addEventListener("click",()=>{const d=o.value.trim();if(d)if(r)try{const u=d.replace(/```json\n?/g,"").replace(/```\n?/g,"").trim(),p=JSON.parse(u);s(p,d)}catch{s(null,d)}else s(d,d)}))}function I(){return a.get("demoMode")}function ze(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Ce(e){const t=a.get("seed"),s=t.refinedResult||null;e.innerHTML=`
    <div class="fade-in">
  ...
  `;const r=e.querySelector("#seedQuestion"),i=e.querySelector("#btnStartChat"),n=e.querySelector("#privacyModal"),o=e.querySelector("#btnOpenPrivacyModal"),l=e.querySelector("#btnClosePrivacyModal");if(o&&o.addEventListener("click",()=>{n.classList.add("visible")}),l&&l.addEventListener("click",()=>{n.classList.remove("visible")}),n&&n.addEventListener("click",c=>{c.target===n&&n.classList.remove("visible")}),r.addEventListener("input",()=>{a.set("seed.question",r.value),i.disabled=!r.value.trim()}),i.addEventListener("click",()=>{De(r.value);const c=e.querySelector("#geminiArea");c&&c.scrollIntoView({behavior:"smooth"})}),s){const c=e.querySelector("#refinedResultArea");_(c,s),R("Theme",(s.theme||s.rq||"").substring(0,60)),R("RQ",s.rq||"")}}function De(e){const t=`${E.rqAssistant}

---

【ユーザーからの研究の種】
${e}`,s=document.querySelector("#geminiChatUI");if(s){if(I()){Ne();return}s.innerHTML=T({prompt:t,containerId:"geminiChat",label:"ブレインストーミング（研究の種の整理）",expectJson:!1,placeholder:"Geminiが出力した「📋 研究の骨子」をここにコピー＆ペーストしてください..."}),A(s,t,r=>{if(r){const i=r||"";let n=Pe(i);if(n||(n=He(i)),n){a.set("seed.refinedResult",n);const o=document.querySelector("#refinedResultArea");o&&(o.classList.remove("hidden"),o.innerHTML=Q(n),_(o,n),o.scrollIntoView({behavior:"smooth"})),R("Theme",(n.theme||n.rq||"").substring(0,60)),R("RQ",n.rq||""),s.innerHTML=`
        ...
        `}else alert("骨子の内容を判読できませんでした。Geminiが出力した「📋 研究の骨子」の部分をそのまま貼り付けてください。")}})}}function Ne(e){const t=JSON.parse(M.rqOverview);a.set("seed.refinedResult",t);const s=document.querySelector("#geminiChatUI");s&&(s.innerHTML=`
      <div class="card" style="background: #f0fdf4; border: 1px solid #86efac; text-align: center; padding: var(--space-4);">
        <p style="color: #166534; font-weight: 600; margin: 0;">✅ デモモード：研究の骨子を自動生成しました。下の内容を確認してください。</p>
      </div>
    `);const r=document.querySelector("#refinedResultArea");r&&(r.classList.remove("hidden"),r.innerHTML=Q(t),_(r,t)),R("Theme",(t.theme||t.rq||"").substring(0,60)),R("RQ",t.rq||"")}function Pe(e){try{const s=e.replace(/```json\s*\n?/g,"").replace(/```\s*\n?/g,"").trim().match(/\{[\s\S]*\}/);if(s){const r=JSON.parse(s[0]);if(r.rq||r.theme||r.type)return r}}catch{}return null}function He(e){const t={type:"research",theme:"",rq:"",target:"",goal:"",approaches:[]},s=e.match(/リサーチクエスチョン[（(]?RQ[)）]?\s*[:：]\s*(.+?)(?:\n|$)/i)||e.match(/RQ\s*[:：]\s*(.+?)(?:\n|$)/i);s&&(t.rq=s[1].trim());const r=e.match(/研究デザイン\s*[:：]\s*(.+?)(?:\n|$)/);r&&(t.theme=r[1].trim());const i=e.match(/対象\s*[:：]\s*(.+?)(?:\n|$)/);i&&(t.target=i[1].trim());const n=e.match(/最終ゴール\s*[:：]\s*(.+?)(?:\n|$)/);n&&(t.goal=n[1].trim()),!t.theme&&t.rq&&(t.theme=t.rq.replace(/[？?]$/,"").substring(0,60));const o=e.matchAll(/(?:量的|質的|STEP\s*\d+|フェーズ)[^:：]*[:：]\s*([\s\S]*?)(?=(?:量的|質的|STEP\s*\d+|フェーズ|最終ゴール|---|\n\n|$))/gi);for(const l of o){const c=l[0].trim(),d=c.match(/^(.+?)[:：]/);d&&t.approaches.push({name:d[1].trim(),description:c.substring(d[0].length).trim().replace(/\n/g," ").substring(0,200)})}return t.rq?t:e.trim().length>30?(t.rq=e.trim().split(`
`)[0].substring(0,200),t.goal=e.trim(),t):null}function Q(e){const t=a.get("seed.rqConfirmed"),s={research:{title:"リサーチクエスチョン",badge:"学術研究"},practice:{title:"実践報告の焦点",badge:"実践報告"},qi:{title:"QIプロジェクト目標",badge:"質改善"}},r=s[e.type]||s.research;return`
  ...
  `}function R(e,t){const s=document.querySelector(`#sum${e}`);s&&(s.textContent=t,s.classList.add("active"))}function je(){return!!a.get("seed.refinedResult")&&!!a.get("seed.rqConfirmed")}function _(e,t){const s=e.querySelector("#btnConfirmRq"),r=e.querySelector("#refinedRqInput");if(!s||!r)return;const i=()=>{r.style.height="auto",r.style.height=Math.max(80,r.scrollHeight+2)+"px"};requestAnimationFrame(i),setTimeout(i,100),r.addEventListener("input",i),s.addEventListener("click",()=>{r.value.trim()&&(t.rq=r.value.trim(),a.set("seed.refinedResult",t),a.set("seed.rqConfirmed",!0),R("Theme",(t.theme||t.title||t.rq).substring(0,60)),R("RQ",t.rq),e.innerHTML=Q(t),_(e,t))})}function Qe(e){const t=a.get("rq"),s=a.get("seed.refinedResult");if(!s){e.innerHTML=`
      <div class="fade-in">
  ...
  `;const o=e.querySelector("#btnGenerateDesign");o?o.addEventListener("click",()=>Se(!1)):r&&K(e)}function Se(e=!1){const t=a.get("seed.refinedResult"),s=a.get("rq.proposalHistory")||[],r=a.get("rq.aiResults");let i=`
...
`)}`}const n=`${E.designSelection}

---

${i}`;if(I()){const l=JSON.parse(M.designSelection);if(e&&r){const d=[...s,r];a.set("rq.proposalHistory",d)}a.set("rq.aiResults",l),a.set("rq.selectedDesign",null);const c=document.querySelector("#designProposalArea");if(c){const d=a.get("rq.proposalHistory")||[];c.innerHTML=F(l,null,d),K(c)}return}const o=document.querySelector("#designProposalArea");o&&(o.innerHTML=T({prompt:n,containerId:"geminiDesign",label:"研究デザイン提案",expectJson:!1,placeholder:"Geminiが出力した「📋 研究デザイン提案」をここにコピー＆ペーストしてください..."}),A(o,n,(l,c)=>{const d=c||l||"";let u=null;try{const m=d.replace(/```json\n?/g,"").replace(/```\n?/g,"").trim().match(/\{[\s\S]*"design"\s*:[\s\S]*\}/);if(m){const g=JSON.parse(m[0]);u=_e(g)}}catch{}if(u||(u=Ge(d)),u){if(e&&r){const m=[...s,r];a.set("rq.proposalHistory",m)}a.set("rq.aiResults",u),a.set("rq.selectedDesign",null);const p=a.get("rq.proposalHistory")||[];o.innerHTML=F(u,null,p),K(o)}else alert("提案内容を判読できませんでした。Geminiが出力した「📋 研究デザイン提案」の部分をそのまま貼り付けてください。")}))}function _e(e){return Array.isArray(e)&&(e=e[0]),e.proposals&&(e=e.proposals[0]),{design:e.design||e.title||e.name||"デザイン提案",vision:e.vision||e.description||e.overview||"",finer:e.finer||{},reason:e.reason||e.recommendation||e.rationale||""}}function Ge(e){e=e.replace(/\*\*(.+?)\*\*/g,"$1").replace(/^[-•]\s*/gm,"").replace(/^\d+\.\s*/gm,"");const t={design:"",vision:"",finer:{},reason:""},s=e.match(/推奨デザイン\s*[:：]\s*(.+?)(?:\n|$)/);s&&(t.design=s[1].trim());const r=e.match(/研究のビジョン\s*[:：]\s*([\s\S]*?)(?=(?:FINER|推奨理由|---|$))/);r&&(t.vision=r[1].trim());const i=e.match(/実現可能性[（(]?F[)）]?\s*[:：]\s*(.+?)(?:\n|$)/);i&&(t.finer.f=i[1].trim());const n=e.match(/面白さ[（(]?I[)）]?\s*[:：]\s*(.+?)(?:\n|$)/);n&&(t.finer.i=n[1].trim());const o=e.match(/新規性[（(]?N[)）]?\s*[:：]\s*(.+?)(?:\n|$)/);o&&(t.finer.n=o[1].trim());const l=e.match(/倫理性[（(]?E[)）]?\s*[:：]\s*(.+?)(?:\n|$)/);l&&(t.finer.e=l[1].trim());const c=e.match(/関連性[（(]?R[)）]?\s*[:：]\s*(.+?)(?:\n|$)/);c&&(t.finer.r=c[1].trim());const d=e.match(/推奨理由\s*[:：]\s*([\s\S]*?)(?=(?:---|✅|📌|$))/);return d&&(t.reason=d[1].trim()),!t.design&&e.trim().length>30&&(t.design=e.trim().split(`
`)[0].substring(0,100)),t.design?t:null}function F(e,t,s){const r=t===e.design,i=Je(e.finer);let n="";return s.length>0&&(n=`
      <div class="card" style="margin-top: var(--space-6); background: var(--color-bg-secondary, #f8f9fa);">
        ...
        <div class="history-list">
          ${s.map((o,l)=>`
            <div class="history-item ${t===o.design?"selected":""}" data-history-index="${l}" data-design="${k(o.design)}">
              <span class="history-number">${l+1}</span>
              <span class="history-title">${k(o.design)}</span>
            </div>
          `).join("")}
        </div>
      </div>
    ...
    <div class="ai-response">
      ...
      <div class="ai-response-body">
        ...
        <div class="proposal-card main-proposal ${r?"selected":""}" data-design="${k(e.design)}" data-is-current="true">
          <div class="proposal-header">
            <span class="badge recommended">★ 推奨デザイン</span>
          </div>
          
          <h3 class="proposal-title">${k(e.design)}</h3>
          ...
          ${e.reason?`
            <div class="proposal-section mt-2">
              <h4>📝 推奨理由</h4>
              <p class="small text-muted">${k(e.reason)}</p>
            </div>
          `:""}
  ...
  `}function Je(e){if(!e||Object.keys(e).length===0)return"";const s=Object.entries({f:{icon:"✅",label:"実現可能性"},i:{icon:"💡",label:"面白さ"},n:{icon:"🆕",label:"新規性"},e:{icon:"🛡️",label:"倫理性"},r:{icon:"🎯",label:"関連性"}}).map(([r,i])=>{const n=e[r];return!n||n===!0?"":`
      <div class="finer-detail-row">
        <span class="finer-label">${i.icon} ${i.label}</span>
        <span class="finer-value">${k(String(n))}</span>
      </div>
    ...
    <div class="fade-in">
      ...
      <div class="card" style="margin-bottom: var(--space-6);">
        ...
        <table class="data-table data-table-selectable" id="designTable">
          ...
          <tbody>
            ${Ue.map(l=>{const c=(s==null?void 0:s.name)===l.guideline&&t===l.type,d=r&&l.recommendFor.includes(r);return`
  ...
  `,s&&ge(s.name),e.querySelectorAll("#designTable tbody tr").forEach(l=>{l.addEventListener("click",()=>{const c=l.dataset.type;l.dataset.guideline,a.set("rq.selectedDesign",c),a.set("guideline.checklist",[]),a.set("guideline.notes",{});const d=N[c]||ve(c);d&&a.set("guideline.selected",d.name),e.querySelectorAll("#designTable tbody tr").forEach(p=>{p.classList.remove("selected-row");const m=p.querySelector("td:last-child");m&&(m.innerHTML="")}),l.classList.add("selected-row");const u=l.querySelector("td:last-child");if(u&&(u.innerHTML='<span class="tag tag-primary">✓ 選択中</span>'),d){const p=e.querySelector("#guidelineDetailArea"),m=j[d.name]||[];p.innerHTML=ue(d,m,{}),me(e),ge(d.name)}Ve("Design",c)})}),me(e)}function ue(e,t,s){const r=Object.values(s).filter(i=>i&&i.trim()).length;return`
  ...
  `:""}function ve(e){if(!e)return null;if(N[e])return N[e];for(const[t,s]of Object.entries(N))if(e.includes(t)||t.includes(e))return s;return N.横断研究}function Ve(e,t){const s=document.querySelector(`#sum${e}`);s&&(s.textContent=t,s.classList.add("active"))}function Ye(){return!!a.get("guideline.selected")}function Xe(e){return j[e]||[]}function Ze(e){const t=a.get("review");e.innerHTML=`
    <div class="fade-in">
      ...
      <div id="step4Results">
        ${t.aiResult?V(t.aiResult):""}
      </div>
    </div>
  `;const s=e.querySelector("#reviewKeywords"),r=e.querySelector("#reviewLang"),i=e.querySelector("#reviewContext");s.addEventListener("input",()=>a.set("review.keywords",s.value)),r.addEventListener("change",()=>a.set("review.language",r.value)),i==null||i.addEventListener("change",()=>a.set("review.context",i.value)),e.querySelector("#btnReview").addEventListener("click",tt);const n=e.querySelector("#btnSuggestQueries");n&&n.addEventListener("click",et),t.suggestedQueries&&W(t.suggestedQueries)}function et(){const e=document.querySelector("#suggestedQueriesArea");if(!e)return;const t=a.get("seed.refinedResult"),s=(t==null?void 0:t.rq)||(t==null?void 0:t.title)||"",r=(t==null?void 0:t.goal)||"",i=(t==null?void 0:t.goal)||"",n=a.get("rq.selectedDesign")||"",o=`
研究テーマ: ${s}
リサーチクエスチョン: ${r}
研究の目的: ${i}
研究デザイン: ${n}
  `.trim(),l=`${E.searchQuerySuggestion}

---

${o}`;if(I()){const c=JSON.parse(M.searchQuerySuggestion);a.set("review.suggestedQueries",c),W(c);const d=document.querySelector("#reviewKeywords");if(d&&!d.value.trim()&&c.keywordsJa){const u=c.keywordsJa.join(" ");d.value=u,a.set("review.keywords",u)}return}e.style.display="block",e.innerHTML=T({prompt:l,containerId:"geminiSuggest",label:"検索キーワード・式の提案",expectJson:!1,placeholder:"Geminiが出力した「📋 検索キーワード」をここにコピー＆ペーストしてください..."}),A(e,l,(c,d)=>{const u=d||c||"";let p=null;try{const g=u.replace(/```json\n?/g,"").replace(/```\n?/g,"").trim().match(/\{[\s\S]*"keywordsJa"[\s\S]*\}/);g&&(p=JSON.parse(g[0]))}catch{}if(p||(p=it(u)),p){a.set("review.suggestedQueries",p),W(p);const m=document.querySelector("#reviewKeywords");if(m&&!m.value.trim()&&p.keywordsJa){const v=(Array.isArray(p.keywordsJa)?p.keywordsJa:[p.keywordsJa]).join(" ");m.value=v,a.set("review.keywords",v)}}else e.innerHTML=`
        <div class="card" style="border-color: var(--color-danger);">
          <p style="color: var(--color-danger);">⚠️ 回答を判読できませんでした。Geminiの回答をそのまま貼り付けてください。</p>
        </div>
      `})}function W(e){const t=document.querySelector("#suggestedQueriesArea");if(!t)return;const s=(e.keywordsJa||[]).join(" "),r=(e.keywordsEn||[]).join(" "),i=`https://pubmed.ncbi.nlm.nih.gov/?term=${encodeURIComponent(r)}`,n=`https://scholar.google.com/scholar?q=${encodeURIComponent(r)}`,o=`https://cir.nii.ac.jp/all?q=${encodeURIComponent(s)}`,l=`https://www.jstage.jst.go.jp/result/global/-char/ja?globalSearchKey=${encodeURIComponent(s)}`,c="display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:4px;font-size:0.78rem;text-decoration:none;border:1px solid;margin-top:6px;margin-right:6px;font-weight:500;";t.style.display="block",t.innerHTML=`
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-4); background: var(--color-primary-bg); padding: var(--space-4); border-radius: var(--radius-sm); border: 1px solid var(--color-primary-border);">
      ...
      <div style="background: var(--color-surface); padding: var(--space-3); border-radius: var(--radius-sm); border: 1px solid var(--color-border);">
        ...
        <div style="margin-top: var(--space-2);">
          ...
          <a href="${o}" target="_blank" rel="noopener" style="${c}color:#1a73e8;border-color:#1a73e8;background:#e8f0fe;">📚 CiNii Research</a>
          <a href="${l}" target="_blank" rel="noopener" style="${c}color:#2e7d32;border-color:#2e7d32;background:#e8f5e9;">📄 J-STAGE</a>
        </div>
      </div>
      ...
      <div style="background: var(--color-surface); padding: var(--space-3); border-radius: var(--radius-sm); border: 1px solid var(--color-border);">
        ...
        <div style="margin-top: var(--space-2);">
          <span style="font-size: 0.75rem; color: var(--color-text-muted);">直接検索:</span><br>
          <a href="${i}" target="_blank" rel="noopener" style="${c}color:#1565c0;border-color:#1565c0;background:#e3f2fd;">🔬 PubMed</a>
          <a href="${n}" target="_blank" rel="noopener" style="${c}color:#4285f4;border-color:#4285f4;background:#e8f0fe;">🎓 Google Scholar</a>
        </div>
      </div>
    </div>
  `}function tt(){const e=a.get("review"),t=a.get("seed.refinedResult"),s=(t==null?void 0:t.rq)||(t==null?void 0:t.title)||"",r=(t==null?void 0:t.goal)||"",i=(t==null?void 0:t.goal)||"",n=a.get("rq.selectedDesign")||"",o=a.get("guideline.selected")||"",l=a.get("guideline.notes")||{},c=Xe(o);let d="";if(c.length>0){const g=[],v=[];c.forEach((S,h)=>{const b=l[h];b&&b.trim()?g.push(`✓ ${S}: ${b.trim()}`):v.push(`□ ${S}`)}),g.length>0&&(d+=`
検討済みの項目:
${g.join(`
`)}`),v.length>0&&(d+=`
未検討の項目（文献レビューで補完が必要）:
...
${d}
  `.trim(),p=`${E.literatureReview}

---

${u}`;if(I()){const g=JSON.parse(M.literatureReview);a.set("review.aiResult",g),document.querySelector("#step4Results").innerHTML=V(g);const v=document.querySelector("#sumLiterature");v&&(v.textContent="背景構築済み",v.classList.add("active"));return}const m=document.querySelector("#step4Results");m.innerHTML=T({prompt:p,containerId:"geminiReview",label:"背景・意義の論理構成",expectJson:!1,placeholder:"Geminiが出力した「📋 研究背景・意義の論理構成案」をここにコピー＆ペーストしてください..."}),A(m,p,(g,v)=>{const S=v||g||"";let h=null;try{const x=S.replace(/```json\n?/g,"").replace(/```\n?/g,"").trim().match(/\{[\s\S]*"structure"[\s\S]*\}/);x&&(h=JSON.parse(x[0]))}catch{}h||(h={structure:S}),a.set("review.aiResult",h),m.innerHTML=V(h);const b=document.querySelector("#sumLiterature");b&&(b.textContent="背景構築済み",b.classList.add("active"))})}function V(e){return`
    <div class="ai-response">
  ...
  `}function st(){return!!a.get("review.aiResult")}function it(e){e=e.replace(/\*\*(.+?)\*\*/g,"$1").replace(/^[-•]\s*/gm,"").replace(/^\d+\.\s*/gm,"");const t={keywordsJa:[],keywordsEn:[],queryJa:"",queryEn:""},s=e.match(/日本語キーワード\s*[:：]\s*(.+?)(?:\n|$)/i);s&&(t.keywordsJa=s[1].split(/[,、，]/).map(o=>o.trim()).filter(o=>o));const r=e.match(/英語キーワード\s*[:：]\s*(.+?)(?:\n|$)/i);r&&(t.keywordsEn=r[1].split(/[,、，]/).map(o=>o.trim()).filter(o=>o));const i=e.match(/日本語検索式\s*[:：]\s*([\s\S]*?)(?=(?:🌍|英語キーワード|英語検索式|---|✅|📌|$))/i);i&&(t.queryJa=i[1].trim());const n=e.match(/英語検索式\s*[:：]\s*([\s\S]*?)(?=(?:---|✅|📌|$))/i);return n&&(t.queryEn=n[1].trim()),t.keywordsJa.length>0||t.keywordsEn.length>0?t:null}const se=[{id:"attributes",label:"基本属性（年齢・性別など）",icon:"👤"},{id:"vitals",label:"バイタルサイン",icon:"💓"},{id:"labs",label:"検査値",icon:"🧪"},{id:"scales",label:"スケール（HADS, BIなど）",icon:"📏"},{id:"observation",label:"観察記録",icon:"👁"},{id:"interview",label:"インタビュー",icon:"🎤"},{id:"questionnaire",label:"アンケート（Likert尺度）",icon:"📋"},{id:"intervention",label:"介入有無",icon:"💊"},{id:"timeseries",label:"時系列データ",icon:"📈"},{id:"medical_record",label:"診療記録・カルテ（看護記録や計画など）",icon:"🏥"},{id:"existing_db",label:"既存データベース利用",icon:"🗄"},{id:"other",label:"その他",icon:"✏️"}],$e=[{value:"none",label:"なし（単群・記述的・実態調査など）"},{value:"2groups",label:"2群（ばく露 vs 非ばく露 / 介入 vs 対照 等）"},{value:"3groups",label:"3群以上"},{value:"prepost",label:"前後比較（同一対象への1つの介入前後など）"},{value:"repeated",label:"反復測定・クロスオーバー（同一対象に対する複数の条件・姿勢・時間での測定など）"},{value:"other",label:"その他"}];function nt(e){const t=a.get("data");e.innerHTML=`
    <div class="fade-in">
      ...
      <div class="card card-highlight" id="dataSummaryPreview">
        ...
        <div id="dataSummaryContent">${xe(t)}</div>
      </div>
    </div>
  `,e.querySelectorAll('#dataTypeGroup input[type="checkbox"]').forEach(i=>{i.addEventListener("change",()=>{const n=Array.from(e.querySelectorAll("#dataTypeGroup input:checked")).map(l=>l.value);a.set("data.types",n),e.querySelectorAll("#dataTypeGroup .checkbox-item").forEach(l=>{l.classList.toggle("checked",l.querySelector("input").checked)});const o=e.querySelector("#typeOtherWrapper");if(o&&(o.style.display=n.includes("other")?"block":"none",!n.includes("other"))){a.set("data.typeOtherText","");const l=e.querySelector("#typeOtherText");l&&(l.value="")}D()})});const s=e.querySelector("#typeOtherText");s&&s.addEventListener("input",i=>{a.set("data.typeOtherText",i.target.value),D()}),e.querySelector("#sampleSize").addEventListener("input",i=>{a.set("data.sampleSize",i.target.value),D()}),e.querySelectorAll('input[name="grouping"]').forEach(i=>{i.addEventListener("change",n=>{a.set("data.grouping",n.target.value),e.querySelectorAll("#groupingGroup .radio-item").forEach(l=>{l.classList.toggle("checked",l.querySelector("input").checked)});const o=e.querySelector("#groupingOtherWrapper");if(o&&(o.style.display=n.target.value==="other"?"block":"none",n.target.value!=="other")){a.set("data.groupingOtherText","");const l=e.querySelector("#groupingOtherText");l&&(l.value="")}D()})});const r=e.querySelector("#groupingOtherText");r&&r.addEventListener("input",i=>{a.set("data.groupingOtherText",i.target.value),D()})}function D(){const e=a.get("data"),t=document.querySelector("#dataSummaryContent");t&&(t.innerHTML=xe(e));const s=document.querySelector("#sumData");if(s){const r=e.types||[];if(r.length>0){const i=r.map(n=>{var o;return((o=se.find(l=>l.id===n))==null?void 0:o.label.split("（")[0])||n});s.textContent=i.join("、"),s.classList.add("active")}}}function xe(e){var r;const t=(e.types||[]).map(i=>se.find(n=>n.id===i));let s=((r=$e.find(i=>i.value===e.grouping))==null?void 0:r.label)||"未選択";return e.grouping==="other"&&e.groupingOtherText&&(s+=`（${fe(e.groupingOtherText)}）`),t.length===0&&!e.sampleSize&&!e.grouping?'<p style="color: var(--color-text-muted);">データを選択すると、ここに概要が表示されます。</p>':`
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-4);">
      <div>
        <strong>データタイプ（${t.length}種類）</strong>
        <ul style="margin-top: var(--space-2); padding-left: var(--space-5);">
          ${t.map(i=>{if(!i)return"";let n=i.label;return i.id==="other"&&e.typeOtherText&&(n+=`（${fe(e.typeOtherText)}）`),`<li>${i.icon} ${n}</li>`}).join("")||'<li style="color: var(--color-text-muted);">未選択</li>'}
        </ul>
      </div>
      <div>
  ...
  `}function rt(){const e=a.get("data");return(e.types||[]).length>0&&e.grouping}function fe(e){return e?e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"):""}const ye={"2groups":{continuous:{normal:"t検定（独立2群）",nonNormal:"Mann-Whitney U検定"},categorical:"χ²検定 / Fisher正確確率検定"},"3groups":{continuous:{normal:"一元配置分散分析（ANOVA）",nonNormal:"Kruskal-Wallis検定"},categorical:"χ²検定"},prepost:{continuous:{normal:"対応のあるt検定",nonNormal:"Wilcoxon符号付順位和検定"},categorical:"McNemar検定"},none:{continuous:{normal:"記述統計、相関分析",nonNormal:"Spearman順位相関"},categorical:"度数分布、記述統計"}};function at(e){const t=a.get("analysis"),s=a.get("data");e.innerHTML=`
    <div class="fade-in">
      ...
      <div id="step6Results">
        ${t.aiResult?Y(t.aiResult):""}
      </div>
    </div>
  `,e.querySelector("#btnAnalysis").addEventListener("click",lt)}function ot(e){var d,u;const t=e.grouping||"none",s=e.types||[],r=ye[t]||ye.none,i=s.some(p=>["vitals","labs","scales"].includes(p)),n=s.some(p=>["attributes","intervention"].includes(p)),o=s.includes("timeseries"),l=s.some(p=>["interview","observation"].includes(p)),c=[];return i&&c.push(`<li><strong>連続変数の群比較:</strong> ${((d=r.continuous)==null?void 0:d.normal)||r.continuous||"記述統計"}（正規分布を仮定）/ ${((u=r.continuous)==null?void 0:u.nonNormal)||"非パラメトリック検定"}（正規分布でない場合）</li>`),n&&c.push(`<li><strong>カテゴリ変数:</strong> ${r.categorical}</li>`),o&&c.push("<li><strong>時系列分析:</strong> 線形混合モデル（Linear Mixed Model）/ 反復測定分散分析</li>"),l&&c.push("<li><strong>質的データ:</strong> 質的内容分析 / テーマ分析 / グラウンデッドセオリー</li>"),e.sampleSize&&parseInt(e.sampleSize)<30&&c.push('<li style="color: var(--color-warning);"><strong>⚠️ 注意:</strong> サンプルサイズが30未満のため、ノンパラメトリック検定の使用を推奨します</li>'),c.length===0?'<p style="color: var(--color-text-muted);">Step 5でデータを選択すると、自動提案が表示されます。</p>':`<ul style="padding-left: var(--space-5);">${c.join("")}</ul>`}function lt(){var n;const e=a.get("data"),s=`
研究デザイン: ${a.get("rq.selectedDesign")||""}
データタイプ: ${(e.types||[]).join(", ")}
サンプルサイズ: ${e.sampleSize||"未定"}
群分け: ${e.grouping||"未定"}
  `.trim(),r=`${E.statisticsProposal}

---

${s}`;if(I()){const o=JSON.parse(M.statisticsProposal);a.set("analysis.aiResult",o),document.querySelector("#step6Results").innerHTML=Y(o);const l=document.querySelector("#sumAnalysis");l&&(l.textContent=((n=o.primaryAnalysis)==null?void 0:n.method)||"提案済み",l.classList.add("active"));return}const i=document.querySelector("#step6Results");i.innerHTML=T({prompt:r,containerId:"geminiAnalysis",label:"統計分析手法の提案",expectJson:!1,placeholder:"Geminiが出力した「📋 分析方法提案」をここにコピー＆ペーストしてください..."}),A(i,r,(o,l)=>{var u;const c=l||o||"";let d=null;try{const m=c.replace(/```json\n?/g,"").replace(/```\n?/g,"").trim().match(/\{[\s\S]*"primaryAnalysis"[\s\S]*\}/);m&&(d=JSON.parse(m[0]))}catch{}if(d||(d=ct(c)),d){a.set("analysis.aiResult",d),i.innerHTML=Y(d);const p=document.querySelector("#sumAnalysis");p&&(p.textContent=((u=d.primaryAnalysis)==null?void 0:u.method)||"提案済み",p.classList.add("active"))}else alert("提案内容を判読できませんでした。Geminiが出力した「📋 分析方法提案」をそのまま貼り付けてください。")})}function ct(e){e=e.replace(/\*\*(.+?)\*\*/g,"$1").replace(/^[-•]\s*/gm,"").replace(/^\d+\.\s*/gm,"");const t={primaryAnalysis:{method:"",reason:""},secondaryAnalyses:[],effectSize:"",multivariateNeeded:!1,multivariateMethod:"",sampleSizeNote:""},s=e.match(/主解析\s*[:：]\s*(.+?)(?:\n|$)/);s&&(t.primaryAnalysis.method=s[1].trim());const r=e.match(/主解析の理由\s*[:：]\s*([\s\S]*?)(?=(?:副解析|効果量|多変量|サンプル|---|✅|📌|$))/);r&&(t.primaryAnalysis.reason=r[1].trim());const i=e.match(/副解析\s*[:：]\s*(.+?)(?:\n|$)/);if(i&&i[1].trim()){const c=e.match(/副解析の理由\s*[:：]\s*([\s\S]*?)(?=(?:効果量|多変量|サンプル|---|✅|📌|$))/);t.secondaryAnalyses.push({method:i[1].trim(),reason:c?c[1].trim():""})}const n=e.match(/効果量\s*[:：]\s*([\s\S]*?)(?=(?:多変量|サンプル|---|✅|📌|$))/);n&&(t.effectSize=n[1].trim());const o=e.match(/多変量解析\s*[:：]\s*([\s\S]*?)(?=(?:サンプル|---|✅|📌|$))/);if(o){const c=o[1].trim();(c.includes("必要")||c.includes("要"))&&(t.multivariateNeeded=!0,t.multivariateMethod=c.replace(/必要[。、．]?\s*/,"").trim())}const l=e.match(/サンプルサイズ概算\s*[:：]\s*([\s\S]*?)(?=(?:---|✅|📌|$))/);return l&&(t.sampleSizeNote=l[1].trim()),t.primaryAnalysis.method?t:e.trim().length>30?(t.primaryAnalysis.method=e.trim().split(`
`)[0].substring(0,100),t.primaryAnalysis.reason=e.trim(),t):null}function Y(e){var t;return`
    <div class="ai-response">
  ...
  `}function dt(){return!!a.get("analysis.aiResult")}function pt(e){const t=a.get("audit.result");e.innerHTML=`
    <div class="fade-in">
  ...
  `,ut();const s=document.getElementById("btnRunAudit");s&&s.addEventListener("click",mt)}function ut(){var l,c,d,u,p,m;const e=document.getElementById("auditInputsSummary");if(!e)return;const t=a.get("seed"),s=a.get("rq"),r=a.get("guideline"),i=a.get("data"),n=a.get("analysis"),o=[{label:"RQ",value:((l=t.refinedResult)==null?void 0:l.rq)||t.question||"未設定",ok:!!((c=t.refinedResult)!=null&&c.rq||t.question)},{label:"研究デザイン",value:s.selectedDesign||"未選択",ok:!!s.selectedDesign},{label:"ガイドライン",value:r.selected||"未選択",ok:!!r.selected},{label:"データタイプ",value:(i.types||[]).join(", ")||"未定",ok:(i.types||[]).length>0},{label:"サンプルサイズ",value:i.sampleSize||"未定",ok:!!i.sampleSize},{label:"主解析",value:((u=(d=n.aiResult)==null?void 0:d.primaryAnalysis)==null?void 0:u.method)||"未提案",ok:!!((m=(p=n.aiResult)==null?void 0:p.primaryAnalysis)!=null&&m.method)}];e.innerHTML=o.map(g=>`
    <div style="display: flex; align-items: center; gap: 8px; padding: 4px 0; border-bottom: 1px solid var(--color-border);">
      <span style="font-size: 0.9rem;">${g.ok?"✅":"⚠️"}</span>
      <span style="font-weight: 500; min-width: 120px;">${g.label}:</span>
      <span style="color: ${g.ok?"var(--color-text)":"var(--color-danger)"};">${g.value}</span>
    </div>
  `).join("")}function mt(){var c,d,u,p,m,g,v,S,h,b,w,x;const e=a.get("seed"),t=a.get("rq"),s=a.get("guideline"),r=a.get("data"),i=a.get("analysis"),n=`
以下の研究計画の論理整合性を監査してください。

【リサーチクエスチョン】
${((c=e.refinedResult)==null?void 0:c.rq)||e.question||"未設定"}

【研究目的】
${((d=e.refinedResult)==null?void 0:d.goal)||"未設定"}

【研究デザイン】
${t.selectedDesign||"未選択"}

【準拠ガイドライン】
${s.selected||"未選択"}

【研究対象】
${((u=e.refinedResult)==null?void 0:u.target)||"未整理"}

【データ収集計画】
データタイプ: ${(r.types||[]).join(", ")||"未定"}
サンプルサイズ: ${r.sampleSize||"未定"}
群分け: ${r.grouping||"未定"}

【分析方法】
主解析: ${((m=(p=i.aiResult)==null?void 0:p.primaryAnalysis)==null?void 0:m.method)||"未提案"}
理由: ${((v=(g=i.aiResult)==null?void 0:g.primaryAnalysis)==null?void 0:v.reason)||""}
副解析: ${(((S=i.aiResult)==null?void 0:S.secondaryAnalyses)||[]).map($=>$.method).join(", ")}
効果量: ${((h=i.aiResult)==null?void 0:h.effectSize)||""}
多変量解析: ${(b=i.aiResult)!=null&&b.multivariateNeeded?(w=i.aiResult)==null?void 0:w.multivariateMethod:"不要"}
サンプルサイズ根拠: ${((x=i.aiResult)==null?void 0:x.sampleSizeNote)||""}
  `.trim(),o=`${E.logicalAudit}

---

${n}`;if(I()){const $=M.logicalAudit;a.set("audit.result",$),document.querySelector("#auditResults").innerHTML=X($);return}const l=document.querySelector("#auditResults");l.innerHTML=T({prompt:o,containerId:"geminiAudit",label:"論理整合性チェック",expectJson:!1,placeholder:"Geminiが出力した「🔎 研究整合性チェック結果」をここにコピー＆ペーストしてください..."}),A(l,o,($,P)=>{const O=P||$||"";a.set("audit.result",O),l.innerHTML=X(O);const z=document.querySelector("#sumAudit");if(z){const H=Re(O);z.textContent=H.label,z.classList.add("active")}})}function Re(e){return e.includes("🔴")?{level:"critical",label:"🔴 要修正",color:"#dc3545"}:e.includes("🟡")?{level:"warning",label:"🟡 軽微修正",color:"#ffc107"}:e.includes("🟢")?{level:"ok",label:"🟢 問題なし",color:"#28a745"}:{level:"unknown",label:"判定不明",color:"#6c757d"}}function X(e){e=e.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>");const t=Re(e),s=e.match(/問題点[\s\S]*?(?=##|修正提案|総合判定|$)/),r=e.match(/修正提案[\s\S]*?(?=総合判定|$)/),i=e.match(/総合判定[:：]?\s*([\s\S]*?)(?:---|$)/),n={critical:"background: linear-gradient(135deg, #f8d7da 0%, #f5c6cb 100%); border-color: #f5c6cb; color: #721c24;",warning:"background: linear-gradient(135deg, #fff3cd 0%, #ffeeba 100%); border-color: #f0c36d; color: #856404;",ok:"background: linear-gradient(135deg, #d4edda 0%, #c3e6cb 100%); border-color: #c3e6cb; color: #155724;",unknown:"background: var(--color-bg-secondary); border-color: var(--color-border); color: var(--color-text);"},o=t.level==="critical"?`
  ...
  `:"";return`
    <div class="ai-response">
      ...
      <div class="ai-response-body">
        ...
        <!-- 総合判定バナー -->
        <div style="border: 2px solid; border-radius: var(--radius-md); padding: var(--space-4); margin-bottom: var(--space-4); ${n[t.level]}">
          <div style="display: flex; align-items: center; gap: var(--space-3);">
            <span style="font-size: 2rem;">${t.level==="critical"?"🔴":t.level==="warning"?"🟡":t.level==="ok"?"🟢":"❓"}</span>
            <div>
              <p style="font-weight: bold; font-size: 1.1rem; margin-bottom: 4px;">総合判定: ${t.label}</p>
              <p style="font-size: var(--font-size-sm);">${i?i[1].replace(/<[^>]*>/g,"").trim():""}</p>
            </div>
          </div>
          ${o}
        </div>
        ...
        ${s?`
          ...
          <div class="card" style="border-left: 4px solid #dc3545; margin-bottom: var(--space-4);">
            <div style="white-space: pre-wrap; line-height: 1.7;">${s[0].replace(/^##?\s*🚨?\s*問題点\s*/m,"").trim()}</div>
          </div>
        `:""}
        ...
        ${r?`
          ...
          <div class="card" style="border-left: 4px solid #007bff; margin-bottom: var(--space-4);">
            <div style="white-space: pre-wrap; line-height: 1.7;">${r[0].replace(/^##?\s*🔧?\s*修正提案\s*/m,"").trim()}</div>
          </div>
        `:""}

      </div>
    </div>
  `}function gt(){return!!a.get("audit.result")}function vt(e){const t=a.get("proposal");e.innerHTML=`
    <div class="fade-in">
  ...
  `,e.querySelector("#btnGenerate").addEventListener("click",yt),t.draft&&ee(t.draft)}function ft(){var o,l;const e=a.get("seed"),t=a.get("rq"),s=a.get("guideline"),r=a.get("review");a.get("data");const i=a.get("analysis"),n=e.refinedResult;return`
    <div>
    ...
    `}function yt(){var b,w,x,$,P,O,z,H,ne,re,ae,oe,le,ce,de,pe;const e=a.get("seed"),t=a.get("rq"),s=a.get("guideline"),r=a.get("review"),i=a.get("data"),n=a.get("analysis"),o=s.selected||"",l=s.notes||{},c=j[o]||[];let d="";if(c.length>0){const f=[];c.forEach((Le,Ee)=>{const B=l[Ee];B&&B.trim()&&f.push(`  ✓ ${Le}: ${B.trim()}`)}),f.length>0&&(d=`

【ガイドライン（${o}）チェックリストに基づくユーザーのメモ・方針】
※ 以下はユーザーがStep 3で入力した研究の方針や検討内容です。計画書作成時にこれらの内容を積極的に反映してください。
${f.join(`
`)}`)}let u=((b=r.aiResult)==null?void 0:b.structure)||"";u.length>600&&(u=u.substring(0,600)+`
…（以下省略。全体の骨格は上記を参考に執筆してください）`);const p=((x=(w=n.aiResult)==null?void 0:w.primaryAnalysis)==null?void 0:x.method)||"未提案",m=(((P=($=n.aiResult)==null?void 0:$.primaryAnalysis)==null?void 0:P.reason)||"").substring(0,200),g=(((O=n.aiResult)==null?void 0:O.secondaryAnalyses)||[]).map(f=>f.method).filter(Boolean).join(", "),v=`
...
【研究テーマ】
${((z=e.refinedResult)==null?void 0:z.theme)||((H=e.refinedResult)==null?void 0:H.title)||"未設定"}

【リサーチクエスチョン】
${((ne=e.refinedResult)==null?void 0:ne.rq)||e.question||"未設定"}

【研究デザイン】
${t.selectedDesign||"未選択"}

【研究の骨子】
対象: ${((re=e.refinedResult)==null?void 0:re.target)||"未整理"}
ゴール: ${((ae=e.refinedResult)==null?void 0:ae.goal)||"未整理"}
アプローチ: ${(((oe=e.refinedResult)==null?void 0:oe.approaches)||[]).map(f=>`${f.name}: ${f.description}`).join(`
`)||"なし"}
...
理由: ${m}
${g?`副解析: ${g}`:""}
効果量: ${((le=n.aiResult)==null?void 0:le.effectSize)||"未決定"}
多変量解析: ${(ce=n.aiResult)!=null&&ce.multivariateNeeded?(de=n.aiResult)==null?void 0:de.multivariateMethod:"不要"}
サンプルサイズ根拠: ${((pe=n.aiResult)==null?void 0:pe.sampleSizeNote)||"未算出"}

━━━ 補足情報 ━━━

【文献レビュー概要（論理構成案）】
${u||"未実施"}
  `.trim(),S=`${E.proposalDraft}

---

${v}`;if(I()){const f=M.proposalDraft;a.set("proposal.draft",f),document.querySelector("#step7Results").innerHTML=Z(f),ee(f);return}const h=document.querySelector("#step7Results");h.innerHTML=T({prompt:S,containerId:"geminiProposal",label:"研究計画書草案の生成",expectJson:!1,placeholder:"Geminiからの研究計画書草案をここに貼り付けてください..."}),A(h,S,f=>{f&&(a.set("proposal.draft",f),h.innerHTML=Z(f),ee(f))})}function ht(e){const t="display:inline-block;background:#fff3cd;color:#856404;font-size:0.7rem;padding:1px 6px;border-radius:3px;margin-left:4px;font-weight:bold;vertical-align:middle;border:1px solid #f0c36d;",s={lit:`<span style="${t}" title="文献を自身で検索し、実在を確認してください">⚠️文献要確認</span>`,stat:`<span style="${t}" title="統計の専門家に妥当性を確認してください">⚠️統計要確認</span>`,sample:`<span style="${t}" title="サンプルサイズの計算根拠を自身で検証してください">⚠️要検証</span>`,ethics:`<span style="${t}" title="施設の倫理審査要件に合わせて修正してください">⚠️施設に合わせて修正</span>`,citation:`<span style="${t}background:#f8d7da;color:#721c24;border-color:#f5c6cb;" title="実在する文献に差し替えてください">🔴出典未確認</span>`};let r=0;const i=n=>(r++,n);return e=e.replace(/出典確認中/g,()=>i(`出典確認中 ${s.citation}`)),e=e.replace(/(先行研究によ(?:れば|ると))/g,n=>i(`${n} ${s.lit}`)),e=e.replace(/(既存の(?:エビデンス|研究|知見)(?:では|によ(?:れば|ると)))/g,n=>i(`${n} ${s.lit}`)),e=e.replace(/((?:国内外の|複数の|多くの|近年の)(?:研究|報告|調査)(?:では|で|が|において))/g,n=>i(`${n} ${s.lit}`)),e=e.replace(/((?:メタアナリシス|システマティックレビュー)(?:では|によ(?:れば|ると)))/g,n=>i(`${n} ${s.lit}`)),e=e.replace(/(t検定|χ²検定|カイ二乗検定|Mann-Whitney|Wilcoxon|ANOVA|分散分析|ロジスティック回帰|Cox比例ハザード|Kaplan-Meier|ログランク検定|線形混合モデル|共分散分析|因子分析|主成分分析|構造方程式)/g,n=>i(`${n} ${s.stat}`)),e=e.replace(/((?:サンプルサイズ|標本サイズ|症例数)(?:は|を|の)[^<]{5,60}?(?:名|例|人|とする|と設定|を目標))/g,n=>i(`${n} ${s.sample}`)),e=e.replace(/(インフォームド・コンセント)/g,n=>i(`${n} ${s.ethics}`)),e=e.replace(/(倫理審査委員会(?:の承認|に申請|の審査))/g,n=>i(`${n} ${s.ethics}`)),{html:e,count:r,tags:s}}function Z(e){let t=e.replace(/^# (.+)$/gm,'<h2 style="margin-top: var(--space-6); color: var(--color-primary-dark); border-bottom: 2px solid var(--color-primary-border); padding-bottom: var(--space-2);">$1</h2>').replace(/^## (.+)$/gm,'<h3 style="margin-top: var(--space-5); color: var(--color-text);">$1</h3>').replace(/^### (.+)$/gm,'<h4 style="margin-top: var(--space-4); color: var(--color-text-secondary);">$1</h4>').replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>").replace(/^- (.+)$/gm,"<li>$1</li>").replace(/\n\n/g,"</p><p>").replace(/\n/g,"<br>");const{html:s,count:r,tags:i}=ht(t);return t=s,`
    <div class="ai-response">
    ...
    <div style="background: var(--color-bg-secondary, #f8f9fa); border: 1px solid var(--color-border); border-radius: var(--radius-md); padding: var(--space-4); margin-bottom: var(--space-4);">
      <div style="display: flex; align-items: center; gap: var(--space-2); margin-bottom: var(--space-3);">
        ...
        <strong style="font-size: 0.95rem;">要確認箇所: ${r}件を自動検出しました</strong>
      </div>
      ...
      <div style="display: flex; flex-wrap: wrap; gap: var(--space-4); font-size: var(--font-size-sm);">
        <span>${i.citation} 出典未確認→実在文献に差替え</span>
        <span>${i.lit} 文献参照→自身で検索・確認</span>
        <span>${i.stat} 統計手法→専門家に確認</span>
        <span>${i.sample} サンプルサイズ→根拠を検証</span>
        <span>${i.ethics} 倫理関連→施設に合わせて修正</span>
      </div>
    </div>
  `:""}
  ...
  `}function ee(e){const t=document.querySelector("#btnCopyProposal");t&&t.addEventListener("click",()=>{navigator.clipboard.writeText(e).then(()=>{t.textContent="✅ コピーしました",setTimeout(()=>{t.textContent="📋 テキストをコピー"},2e3)}).catch(()=>{const i=document.createElement("textarea");i.value=e,document.body.appendChild(i),i.select(),document.execCommand("copy"),document.body.removeChild(i),t.textContent="✅ コピーしました",setTimeout(()=>{t.textContent="📋 テキストをコピー"},2e3)})});const s=document.querySelector("#btnDownloadWord");s&&s.addEventListener("click",()=>{bt(e)});const r=document.querySelector("#btnDownloadPDF");r&&r.addEventListener("click",()=>{St(e)})}function bt(e){var l,c,d,u;const s=`
<!DOCTYPE html>
...
<body>
  ${t}
...
</html>`),s.document.close(),setTimeout(()=>{s.print()},500)}function $t(){return!!a.get("proposal.draft")}const xt=[{id:1,render:Ce,validate:je},{id:2,render:Qe,validate:Be},{id:3,render:Fe,validate:Ye},{id:4,render:Ze,validate:st},{id:5,render:nt,validate:rt},{id:6,render:at,validate:dt},{id:7,render:pt,validate:gt},{id:8,render:vt,validate:$t}],Rt=document.getElementById("stepContent"),wt=document.getElementById("progressFill"),we=document.querySelectorAll(".step-tab"),qe=document.getElementById("btnPrev"),L=document.getElementById("btnNext"),qt=document.getElementById("stepIndicator"),C=document.getElementById("settingsModal");let y=a.get("currentStep")||1,he=!1;function ke(){he||(he=!0,a.hasSavedData()&&At(),G(),J(),Tt(),Et(),Mt(),It(),ie())}function G(){const e=xt.find(t=>t.id===y);e&&e.render(Rt)}const kt={1:{label:"種と整理",affects:[2,4,6,7,8],dataKeys:["seed.refinedResult"]},2:{label:"デザイン案",affects:[3,4,6,7,8],dataKeys:["rq.selectedDesign","rq.aiResults"]},3:{label:"ガイドライン",affects:[4,8],dataKeys:["guideline.selected"]},4:{label:"文献レビュー",affects:[8],dataKeys:["review.aiResult"]},5:{label:"データ収集",affects:[6,7,8],dataKeys:["data.types","data.sampleSize"]},6:{label:"分析方法",affects:[7,8],dataKeys:["analysis.aiResult"]},7:{label:"整合性チェック",affects:[8],dataKeys:["audit.result"]}},be={1:"種と整理",2:"デザイン案",3:"ガイドライン",4:"文献レビュー",5:"データ収集",6:"分析方法",7:"整合性チェック",8:"計画書草案"};function te(e){if(!(e<1||e>8)){if(e<y){const t=kt[e];if(t){const s=t.affects.filter(r=>r===2?!!a.get("rq.aiResults"):r===3?!!a.get("guideline.selected"):r===4?!!a.get("review.aiResult"):r===6?!!a.get("analysis.aiResult"):r===7?!!a.get("audit.result"):r===8?!!a.get("proposal.draft"):!1);s.length>0&&Lt(e,s)}}y=e,a.set("currentStep",y),G(),J(),window.scrollTo({top:0,behavior:"smooth"})}}function Lt(e,t){const s=t.map(n=>`Step ${n}（${be[n]}）`).join("、"),r=document.getElementById("staleDataWarning");r&&r.remove();const i=document.createElement("div");i.id="staleDataWarning",i.style.cssText="position:fixed;top:0;left:0;right:0;z-index:9999;background:linear-gradient(135deg,#fff3cd,#ffeeba);border-bottom:2px solid #f0c36d;padding:12px 20px;display:flex;align-items:center;gap:12px;box-shadow:0 2px 8px rgba(0,0,0,0.1);",i.innerHTML=`
      <span style="font-size:1.3rem;">🔄</span>
      <div style="flex:1;color:#856404;font-size:0.9rem;">
        <strong>Step ${e}（${be[e]}）を修正すると、${s} の内容が古くなる可能性があります。</strong><br>
    ...
    `,document.body.prepend(i),setTimeout(()=>{i.parentElement&&i.remove()},1e4)}function J(){const e=y/8*100;wt.style.width=`${e}%`,we.forEach(t=>{const s=parseInt(t.dataset.step);t.classList.toggle("active",s===y),t.classList.toggle("completed",a.get("completedSteps").has(s))}),qe.disabled=y===1,y===8?(L.innerHTML="✅ 完了",L.classList.remove("btn-primary"),L.classList.add("btn-success")):(L.innerHTML=`
      確認して次へ
    ...
    `,L.classList.remove("btn-success"),L.classList.add("btn-primary")),qt.textContent=`Step ${y} / 8`}function Et(){we.forEach(e=>{e.addEventListener("click",()=>{const t=parseInt(e.dataset.step);te(t)})})}function Mt(){qe.addEventListener("click",()=>{te(y-1)}),L.addEventListener("click",()=>{const e=a.get("completedSteps");e.add(y),a.set("completedSteps",e),y<8&&te(y+1)})}function Tt(){const e=document.getElementById("btnSettings"),t=document.getElementById("btnCloseSettings"),s=document.getElementById("btnSaveSettings"),r=document.getElementById("demoModeToggle");r.checked=a.get("demoMode"),e.addEventListener("click",()=>{C.classList.add("visible")}),t.addEventListener("click",()=>{C.classList.remove("visible")}),C.addEventListener("click",i=>{i.target===C&&C.classList.remove("visible")}),s.addEventListener("click",()=>{a.saveDemoMode(r.checked),C.classList.remove("visible"),s.textContent="✅ 保存しました",setTimeout(()=>{s.textContent="保存"},1500)})}function ie(){var o,l,c,d,u,p;const e=a.get("seed"),t=a.get("rq"),s=a.get("guideline"),r=a.get("review"),i=a.get("data"),n=a.get("analysis");((o=e.refinedResult)!=null&&o.theme||(l=e.refinedResult)!=null&&l.title||e.question)&&q("Theme",(((c=e.refinedResult)==null?void 0:c.theme)||((d=e.refinedResult)==null?void 0:d.title)||e.question).substring(0,60)),e.refinedResult&&q("RQ",e.refinedResult.rq||e.refinedResult.title),t.selectedDesign&&q("Design",t.selectedDesign),s.selected&&q("Guideline",s.selected),r.aiResult&&q("Literature","背景構築済み"),((u=i.types)==null?void 0:u.length)>0&&q("Data",`${i.types.length}種類のデータ`),(p=n.aiResult)!=null&&p.primaryAnalysis&&q("Analysis",n.aiResult.primaryAnalysis.method)}function q(e,t){const s=document.querySelector(`#sum${e}`);s&&(s.textContent=t,s.classList.add("active"))}function At(){const e=document.createElement("div");e.className="modal-overlay visible",e.id="resumeDialog",e.innerHTML=`
    ...
    `,document.body.appendChild(e),document.getElementById("btnResume").addEventListener("click",()=>{a.loadFullState(),y=a.get("currentStep")||1,G(),J(),ie(),e.remove()}),document.getElementById("btnNewProject").addEventListener("click",()=>{a.reset()})}function It(){const e=document.getElementById("btnExport"),t=document.getElementById("btnImport"),s=document.getElementById("fileImport");e&&e.addEventListener("click",()=>{a.exportToJSON();const r=e.innerHTML;e.innerHTML="✅ 保存しました",setTimeout(()=>{e.innerHTML=r},2e3)}),t&&s&&(t.addEventListener("click",()=>{s.click()}),s.addEventListener("change",async r=>{const i=r.target.files[0];if(i){try{await a.importFromJSON(i),y=a.get("currentStep")||1,G(),J(),ie();const n=t.innerHTML;t.innerHTML="✅ 読み込み完了",setTimeout(()=>{t.innerHTML=n},2e3)}catch(n){alert("ファイルの読み込みに失敗しました: "+n.message)}s.value=""}}))}document.addEventListener("DOMContentLoaded",ke);document.readyState!=="loading"&&ke();
