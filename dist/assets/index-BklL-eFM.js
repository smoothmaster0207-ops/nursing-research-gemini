...
Ⅴ. 期待される成果
看護師の業務負担を最適化しつつ、患者の安全な在宅移行を実現するためのエビデンスを提示する。`},xe="https://gemini.google.com/app";async function qe(e){try{return await navigator.clipboard.writeText(e),!0}catch{const s=document.createElement("textarea");s.value=e,s.style.position="fixed",s.style.opacity="0",document.body.appendChild(s),s.select();try{return document.execCommand("copy"),document.body.removeChild(s),!0}catch{return document.body.removeChild(s),!1}}}function Le(){window.open(xe,"_blank","noopener,noreferrer")}function k(e){const{prompt:t,containerId:s,label:r="AI回答",expectJson:i=!1,placeholder:n="Geminiからの回答をここに貼り付けてください..."}=e;return`
...
【ユーザーからの研究の種】
${e}`,s=document.querySelector("#geminiChatUI");if(s){if(M()){Me();return}s.innerHTML=k({prompt:t,containerId:"geminiChat",label:"ブレインストーミング（研究の種の整理）",expectJson:!1,placeholder:"Geminiが出力した「📋 研究の骨子」をここにコピー＆ペーストしてください..."}),T(s,t,r=>{if(r){const i=r||"";let n=Ae(i);if(n||(n=Ie(i)),n){a.set("seed.refinedResult",n);const o=document.querySelector("#refinedResultArea");o&&(o.classList.remove("hidden"),o.innerHTML=N(n),D(o,n),o.scrollIntoView({behavior:"smooth"})),$("Theme",(n.theme||n.rq||"").substring(0,60)),$("RQ",n.rq||""),s.innerHTML=`
...
`)[0].substring(0,100)),t.design?t:null}function G(e,t,s){const r=t===e.design,i=De(e.finer);let n="";return s.length>0&&(n=`
  ...
  `:""}function de(e){if(!e)return null;if(O[e])return O[e];for(const[t,s]of Object.entries(O))if(e.includes(t)||t.includes(e))return s;return O.横断研究}function je(e,t){const s=document.querySelector(`#sum${e}`);s&&(s.textContent=t,s.classList.add("active"))}function Je(){return!!a.get("guideline.selected")}function Fe(e){return z[e]||[]}function Be(e){const t=a.get("review");e.innerHTML=`
    <div class="fade-in">
...
${o}`;if(M()){const c=JSON.parse(E.searchQuerySuggestion);a.set("review.suggestedQueries",c),J(c);const d=document.querySelector("#reviewKeywords");if(d&&!d.value.trim()&&c.keywordsJa){const u=c.keywordsJa.join(" ");d.value=u,a.set("review.keywords",u)}return}e.style.display="block",e.innerHTML=k({prompt:l,containerId:"geminiSuggest",label:"検索キーワード・式の提案",expectJson:!1,placeholder:"Geminiが出力した「📋 検索キーワード」をここにコピー＆ペーストしてください..."}),T(e,l,(c,d)=>{const u=d||c||"";let p=null;try{const g=u.replace(/```json\n?/g,"").replace(/```\n?/g,"").trim().match(/\{[\s\S]*"keywordsJa"[\s\S]*\}/);g&&(p=JSON.parse(g[0]))}catch{}if(p||(p=Ye(u)),p){a.set("review.suggestedQueries",p),J(p);const m=document.querySelector("#reviewKeywords");if(m&&!m.value.trim()&&p.keywordsJa){const h=(Array.isArray(p.keywordsJa)?p.keywordsJa:[p.keywordsJa]).join(" ");m.value=h,a.set("review.keywords",h)}}else e.innerHTML=`
  ...
  `)}function We(){const e=a.get("review"),t=a.get("seed.refinedResult"),s=(t==null?void 0:t.rq)||(t==null?void 0:t.title)||"",r=(t==null?void 0:t.goal)||"",i=(t==null?void 0:t.goal)||"",n=a.get("rq.selectedDesign")||"",o=a.get("guideline.selected")||"",l=a.get("guideline.notes")||{},c=Fe(o);let d="";if(c.length>0){const g=[],h=[];c.forEach((S,f)=>{const b=l[f];b&&b.trim()?g.push(`✓ ${S}: ${b.trim()}`):h.push(`□ ${S}`)}),g.length>0&&(d+=`
検討済みの項目:
${g.join(`
`)}`),h.length>0&&(d+=`
未検討の項目（文献レビューで補完が必要）:
...
${u}`;if(M()){const g=JSON.parse(E.literatureReview);a.set("review.aiResult",g),document.querySelector("#step4Results").innerHTML=F(g);const h=document.querySelector("#sumLiterature");h&&(h.textContent="背景構築済み",h.classList.add("active"));return}const m=document.querySelector("#step4Results");m.innerHTML=k({prompt:p,containerId:"geminiReview",label:"背景・意義の論理構成",expectJson:!1,placeholder:"Geminiが出力した「📋 研究背景・意義の論理構成案」をここにコピー＆ペーストしてください..."}),T(m,p,(g,h)=>{const S=h||g||"";let f=null;try{const A=S.replace(/```json\n?/g,"").replace(/```\n?/g,"").trim().match(/\{[\s\S]*"structure"[\s\S]*\}/);A&&(f=JSON.parse(A[0]))}catch{}f||(f={structure:S}),a.set("review.aiResult",f),m.innerHTML=F(f);const b=document.querySelector("#sumLiterature");b&&(b.textContent="背景構築済み",b.classList.add("active"))})}function F(e){return`
    <div class="ai-response">
  ...
  `}function Ke(){return!!a.get("review.aiResult")}function Ye(e){const t={keywordsJa:[],keywordsEn:[],queryJa:"",queryEn:""},s=e.match(/日本語キーワード\s*[:：]\s*(.+?)(?:\n|$)/i);s&&(t.keywordsJa=s[1].split(/[,、，]/).map(o=>o.trim()).filter(o=>o));const r=e.match(/英語キーワード\s*[:：]\s*(.+?)(?:\n|$)/i);r&&(t.keywordsEn=r[1].split(/[,、，]/).map(o=>o.trim()).filter(o=>o));const i=e.match(/日本語検索式\s*[:：]\s*([\s\S]*?)(?=(?:🌍|英語キーワード|英語検索式|---|✅|📌|$))/i);i&&(t.queryJa=i[1].trim());const n=e.match(/英語検索式\s*[:：]\s*([\s\S]*?)(?=(?:---|✅|📌|$))/i);return n&&(t.queryEn=n[1].trim()),t.keywordsJa.length>0||t.keywordsEn.length>0?t:null}const Y=[{id:"attributes",label:"基本属性（年齢・性別など）",icon:"👤"},{id:"vitals",label:"バイタルサイン",icon:"💓"},{id:"labs",label:"検査値",icon:"🧪"},{id:"scales",label:"スケール（HADS, BIなど）",icon:"📏"},{id:"observation",label:"観察記録",icon:"👁"},{id:"interview",label:"インタビュー",icon:"🎤"},{id:"questionnaire",label:"アンケート（Likert尺度）",icon:"📋"},{id:"intervention",label:"介入有無",icon:"💊"},{id:"timeseries",label:"時系列データ",icon:"📈"},{id:"medical_record",label:"診療記録・カルテ（看護記録や計画など）",icon:"🏥"},{id:"existing_db",label:"既存データベース利用",icon:"🗄"},{id:"other",label:"その他",icon:"✏️"}],ve=[{value:"none",label:"なし（単群・記述的・実態調査など）"},{value:"2groups",label:"2群（ばく露 vs 非ばく露 / 介入 vs 対照 等）"},{value:"3groups",label:"3群以上"},{value:"prepost",label:"前後比較（同一対象への1つの介入前後など）"},{value:"repeated",label:"反復測定・クロスオーバー（同一対象に対する複数の条件・姿勢・時間での測定など）"},{value:"other",label:"その他"}];function Ve(e){const t=a.get("data");e.innerHTML=`
    <div class="fade-in">
      ...
      <div class="card card-highlight" id="dataSummaryPreview">
        ...
        <div id="dataSummaryContent">${he(t)}</div>
      </div>
    </div>
  `,e.querySelectorAll('#dataTypeGroup input[type="checkbox"]').forEach(i=>{i.addEventListener("change",()=>{const n=Array.from(e.querySelectorAll("#dataTypeGroup input:checked")).map(l=>l.value);a.set("data.types",n),e.querySelectorAll("#dataTypeGroup .checkbox-item").forEach(l=>{l.classList.toggle("checked",l.querySelector("input").checked)});const o=e.querySelector("#typeOtherWrapper");if(o&&(o.style.display=n.includes("other")?"block":"none",!n.includes("other"))){a.set("data.typeOtherText","");const l=e.querySelector("#typeOtherText");l&&(l.value="")}I()})});const s=e.querySelector("#typeOtherText");s&&s.addEventListener("input",i=>{a.set("data.typeOtherText",i.target.value),I()}),e.querySelector("#sampleSize").addEventListener("input",i=>{a.set("data.sampleSize",i.target.value),I()}),e.querySelectorAll('input[name="grouping"]').forEach(i=>{i.addEventListener("change",n=>{a.set("data.grouping",n.target.value),e.querySelectorAll("#groupingGroup .radio-item").forEach(l=>{l.classList.toggle("checked",l.querySelector("input").checked)});const o=e.querySelector("#groupingOtherWrapper");if(o&&(o.style.display=n.target.value==="other"?"block":"none",n.target.value!=="other")){a.set("data.groupingOtherText","");const l=e.querySelector("#groupingOtherText");l&&(l.value="")}I()})});const r=e.querySelector("#groupingOtherText");r&&r.addEventListener("input",i=>{a.set("data.groupingOtherText",i.target.value),I()})}function I(){const e=a.get("data"),t=document.querySelector("#dataSummaryContent");t&&(t.innerHTML=he(e));const s=document.querySelector("#sumData");if(s){const r=e.types||[];if(r.length>0){const i=r.map(n=>{var o;return((o=Y.find(l=>l.id===n))==null?void 0:o.label.split("（")[0])||n});s.textContent=i.join("、"),s.classList.add("active")}}}function he(e){var r;const t=(e.types||[]).map(i=>Y.find(n=>n.id===i));let s=((r=ve.find(i=>i.value===e.grouping))==null?void 0:r.label)||"未選択";return e.grouping==="other"&&e.groupingOtherText&&(s+=`（${pe(e.groupingOtherText)}）`),t.length===0&&!e.sampleSize&&!e.grouping?'<p style="color: var(--color-text-muted);">データを選択すると、ここに概要が表示されます。</p>':`
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-4);">
      <div>
...
`)[0].substring(0,100),t.primaryAnalysis.reason=e.trim(),t):null}function B(e){var t;return`
  ...
  `,e.querySelector("#btnGenerate").addEventListener("click",at),t.draft&&W(t.draft)}function rt(){var o,l;const e=a.get("seed"),t=a.get("rq"),s=a.get("guideline"),r=a.get("review");a.get("data");const i=a.get("analysis"),n=e.refinedResult;return`
    ...
    `}function at(){var g,h,S,f,b,C,A,X,Z,ee,te,se,ie,ne,re,ae;const e=a.get("seed"),t=a.get("rq"),s=a.get("guideline"),r=a.get("review"),i=a.get("data"),n=a.get("analysis"),o=s.selected||"",l=s.notes||{},c=z[o]||[];let d="";if(c.length>0){const v=[];c.forEach((Se,$e)=>{const _=l[$e];_&&_.trim()&&v.push(`  ✓ ${Se}: ${_.trim()}`)}),v.length>0&&(d=`
...
【研究テーマ】
${((g=e.refinedResult)==null?void 0:g.theme)||((h=e.refinedResult)==null?void 0:h.title)||""}
...
【研究の骨子】
対象: ${((f=e.refinedResult)==null?void 0:f.target)||"未整理"}
...
${u}`;if(M()){const v=E.proposalDraft;a.set("proposal.draft",v),document.querySelector("#step7Results").innerHTML=U(v),W(v);return}const m=document.querySelector("#step7Results");m.innerHTML=k({prompt:p,containerId:"geminiProposal",label:"研究計画書草案の生成",expectJson:!1,placeholder:"Geminiからの研究計画書草案をここに貼り付けてください..."}),T(m,p,v=>{v&&(a.set("proposal.draft",v),m.innerHTML=U(v),W(v))})}function U(e){return`
    <div class="ai-response">
  ...
  `}function W(e){const t=document.querySelector("#btnCopyProposal");t&&t.addEventListener("click",()=>{navigator.clipboard.writeText(e).then(()=>{t.textContent="✅ コピーしました",setTimeout(()=>{t.textContent="📋 テキストをコピー"},2e3)}).catch(()=>{const i=document.createElement("textarea");i.value=e,document.body.appendChild(i),i.select(),document.execCommand("copy"),document.body.removeChild(i),t.textContent="✅ コピーしました",setTimeout(()=>{t.textContent="📋 テキストをコピー"},2e3)})});const s=document.querySelector("#btnDownloadWord");s&&s.addEventListener("click",()=>{ot(e)});const r=document.querySelector("#btnDownloadPDF");r&&r.addEventListener("click",()=>{lt(e)})}function ot(e){var l,c,d,u;const s=`
<!DOCTYPE html>
...
<body>
  ${t}
...
</html>`),s.document.close(),setTimeout(()=>{s.print()},500)}function ct(){return!!a.get("proposal.draft")}const dt=[{id:1,render:ke,validate:Oe},{id:2,render:Ce,validate:Pe},{id:3,render:_e,validate:Je},{id:4,render:Be,validate:Ke},{id:5,render:Ve,validate:Xe},{id:6,render:Ze,validate:it},{id:7,render:nt,validate:ct}],pt=document.getElementById("stepContent"),ut=document.getElementById("progressFill"),ye=document.querySelectorAll(".step-tab"),fe=document.getElementById("btnPrev"),x=document.getElementById("btnNext"),mt=document.getElementById("stepIndicator"),q=document.getElementById("settingsModal");let y=a.get("currentStep")||1,me=!1;function be(){me||(me=!0,a.hasSavedData()&&yt(),P(),H(),ht(),gt(),vt(),ft(),V())}function P(){const e=dt.find(t=>t.id===y);e&&e.render(pt)}function K(e){e<1||e>7||(y=e,a.set("currentStep",y),P(),H(),window.scrollTo({top:0,behavior:"smooth"}))}function H(){const e=y/7*100;ut.style.width=`${e}%`,ye.forEach(t=>{const s=parseInt(t.dataset.step);t.classList.toggle("active",s===y),t.classList.toggle("completed",a.get("completedSteps").has(s))}),fe.disabled=y===1,y===7?(x.innerHTML="✅ 完了",x.classList.remove("btn-primary"),x.classList.add("btn-success")):(x.innerHTML=`
      確認して次へ
    ...
    `,x.classList.remove("btn-success"),x.classList.add("btn-primary")),mt.textContent=`Step ${y} / 7`}function gt(){ye.forEach(e=>{e.addEventListener("click",()=>{const t=parseInt(e.dataset.step);K(t)})})}function vt(){fe.addEventListener("click",()=>{K(y-1)}),x.addEventListener("click",()=>{const e=a.get("completedSteps");e.add(y),a.set("completedSteps",e),y<7&&K(y+1)})}function ht(){const e=document.getElementById("btnSettings"),t=document.getElementById("btnCloseSettings"),s=document.getElementById("btnSaveSettings"),r=document.getElementById("demoModeToggle");r.checked=a.get("demoMode"),e.addEventListener("click",()=>{q.classList.add("visible")}),t.addEventListener("click",()=>{q.classList.remove("visible")}),q.addEventListener("click",i=>{i.target===q&&q.classList.remove("visible")}),s.addEventListener("click",()=>{a.saveDemoMode(r.checked),q.classList.remove("visible"),s.textContent="✅ 保存しました",setTimeout(()=>{s.textContent="保存"},1500)})}function V(){var o,l,c,d,u,p;const e=a.get("seed"),t=a.get("rq"),s=a.get("guideline"),r=a.get("review"),i=a.get("data"),n=a.get("analysis");((o=e.refinedResult)!=null&&o.theme||(l=e.refinedResult)!=null&&l.title||e.question)&&R("Theme",(((c=e.refinedResult)==null?void 0:c.theme)||((d=e.refinedResult)==null?void 0:d.title)||e.question).substring(0,60)),e.refinedResult&&R("RQ",e.refinedResult.rq||e.refinedResult.title),t.selectedDesign&&R("Design",t.selectedDesign),s.selected&&R("Guideline",s.selected),r.aiResult&&R("Literature","背景構築済み"),((u=i.types)==null?void 0:u.length)>0&&R("Data",`${i.types.length}種類のデータ`),(p=n.aiResult)!=null&&p.primaryAnalysis&&R("Analysis",n.aiResult.primaryAnalysis.method)}function R(e,t){const s=document.querySelector(`#sum${e}`);s&&(s.textContent=t,s.classList.add("active"))}function yt(){const e=document.createElement("div");e.className="modal-overlay visible",e.id="resumeDialog",e.innerHTML=`
